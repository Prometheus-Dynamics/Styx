...
fn docker_capture_virtual_reports_virtual_frames() {
    ...
    let output = facade.run_output("/workspace/target/debug/quickstart_capture_virtual");
    assert!(
...
fn docker_low_latency_preview_reports_pipeline_health() {
    let facade = DockerFacade::start();
    let output = facade.run_output("/workspace/target/debug/low_latency_preview");
    assert!(
        output.status.success(),
...
fn docker_async_pipeline_reports_async_samples() {
    let facade = DockerFacade::start();
    let output = facade.run_output("/workspace/target/debug/async_pipeline");
    assert!(
        output.status.success(),
...
fn docker_reliable_recording_writes_frames() {
    ...
    let output = facade.run_output(
        "rm -rf /tmp/styx-recordings && /workspace/target/debug/reliable_recording /tmp/styx-recordings 6 && test -f /tmp/styx-recordings/frame_000000.png && echo frame_000000.png:ok",
    );
    assert!(
        output.status.success(),
