//! Bayer demosaic implementations.
//!
//! This module intentionally keeps the scalar, u16, and platform-optimized paths
//! together so the Bayer sampling rules and edge handling stay in one place:
//! - `demosaic_bilinear_to_rg24` is the byte-oriented scalar fallback.
//! - `demosaic_bilinear_u16_le` handles unpacked little-endian 10/12/16-bit input.
//! - `demosaic_bilinear_*_neon` functions are architecture-specific hot paths.
//!
//! The loops are performance-sensitive and keep indexing explicit; clippy
//! allowances below should remain local to those hot loops.
...
// Hot-path demosaic code keeps indexing explicit so LLVM can optimize the row loops.
...
// Explicit channel loops keep the interpolation math close to the Bayer layout.
...
pub(super) fn demosaic_bilinear_u16_le(
    ...
    dst_inner
        ...
        .for_each(|(row_idx, out_row)| {
            ...
            #[cfg(target_arch = "aarch64")]
            unsafe {
                use self::neon::{avg2_u16, avg4_u16, shift_u16x8_to_u8};
