...
fn map_colorspace(color: ColorSpace) -> (YuvRange, YuvStandardMatrix) {
    match color {
        ...
        // See `nv12.rs`: `Srgb` is used as a "full-range output" hint for camera YUV.
...
pub struct Yuv420pToRgbDecoder {
    descriptor: CodecDescriptor,
    pool: BufferPool,
    uv_order: UvOrder,
}

impl Yuv420pToRgbDecoder {
    pub fn new(
        input: FourCc,
    ...
    pub fn decode_into(&self, input: &FrameLease, dst: &mut [u8]) -> Result<FrameMeta, CodecError> {
        ...
        if planes.len() < 3 {
            return Err(CodecError::Codec("yuv420p frame missing planes".into()));
        }
        let y_plane = &planes[0];
        let (u_plane, v_plane) = match self.uv_order {
            UvOrder::Uv => (&planes[1], &planes[2]),
            UvOrder::Vu => (&planes[2], &planes[1]),
        };
        ...
        let y_required = y_stride
            .checked_mul(height)
            .ok_or_else(|| CodecError::Codec("yuv420p y stride overflow".into()))?;
        let u_required = u_stride
            .checked_mul(chroma_height)
            .ok_or_else(|| CodecError::Codec("yuv420p u stride overflow".into()))?;
        let v_required = v_stride
            .checked_mul(chroma_height)
            .ok_or_else(|| CodecError::Codec("yuv420p v stride overflow".into()))?;
        if y_plane.data().len() < y_required
            || u_plane.data().len() < u_required
            || v_plane.data().len() < v_required
        {
            return Err(CodecError::Codec("yuv420p plane buffer too short".into()));
        }

        let row_bytes = width
            .checked_mul(3)
            .ok_or_else(|| CodecError::Codec("yuv420p output overflow".into()))?;
        let out_len = row_bytes
            .checked_mul(height)
            .ok_or_else(|| CodecError::Codec("yuv420p output overflow".into()))?;
        if dst.len() < out_len {
            return Err(CodecError::Codec("yuv420p dst buffer too short".into()));
        }
...
impl Codec for Yuv420pToRgbDecoder {
...
#[cfg(feature = "image")]
impl ImageDecode for Yuv420pToRgbDecoder {
